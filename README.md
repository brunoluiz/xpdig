...
# Loading a trace generated by `crossplane resource trace -o json <>`
cat <trace.json> | xpdig trace --stdin
crossplane resource trace -o json <> | xpdig trace --stdin
```

### `crossplane beta trace` deprecation

In May 2026, Crossplane promoted the `trace` command to stable, which changed the tool call from
`crossplane beta trace` to `crossplane resource trace`. If for some reason you can't run the latest version of
the CLI, you can still use `xpdig` with the old command by setting `--cmd "crossplane beta trace -o json"`
when calling it. The same is valid to the `k9s` integration.

```bash
xpdig trace --context <context> --cmd "crossplane beta trace -o json" Object/hello-world
```
